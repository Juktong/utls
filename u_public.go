...
type TLS13OnlyState struct {
	...
	// KeyShareKeysByGroup contains ephemeral private key material for key shares
	// generated by ApplyPreset, keyed by the group advertised in the
	// ClientHello. It is used to select KeyShareKeys after the final ServerHello
	// and is not retained when the private handshake state is converted back.
	// Callers that supply KeyShare.Data manually must manage the corresponding
	// private key material themselves.
...
func keySharePrivateKeysMatchGroup(keyShareKeys *keySharePrivateKeys, group CurveID) bool {
	if keyShareKeys == nil {
		return false
	}
	if keyShareKeys.curveID == group {
		return true
	}
	...
	curveID, ok := curveIDForCurve(keyShareKeys.ecdhe.Curve())
	return ok && curveID == group
}

// selectKeyShareKeysForGroup selects the private key material matching the
// group in the final ServerHello. It checks the active private state first so
// keys generated while processing a HelloRetryRequest take precedence over the
// original ClientHello registry.
func (hs *clientHandshakeStateTLS13) selectKeyShareKeysForGroup(group CurveID) {
	if keySharePrivateKeysMatchGroup(hs.keyShareKeys, group) || hs.uconn == nil {
		return
	}

	keyShareKeys := hs.uconn.HandshakeState.State13.KeyShareKeysByGroup[group]
	if keyShareKeys == nil {
		// A missing entry can represent a caller-supplied KeyShare.Data. Preserve
		// the existing private material and let the standard key-exchange checks
		// report any mismatch.
		return
	}
	hs.keyShareKeys = keyShareKeys.ToPrivate()
}
